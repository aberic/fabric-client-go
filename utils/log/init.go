...
func init() {
	logFileDir = gnomon.Env().GetD(utils.LogDirEnv, os.TempDir())
	logFileMaxSize = gnomon.Env().GetIntD(utils.LogFileMaxSizeEnv, 1024)
	logFileMaxAge = gnomon.Env().GetIntD(utils.LogFileMaxAgeEnv, 7)
	logUtc = gnomon.Env().GetBool(utils.LogUtcEnv)
	logLevel = gnomon.Env().GetD(utils.LogLevelEnv, "Debug")
	logProduction = gnomon.Env().GetBool(utils.LogProductionEnv)
