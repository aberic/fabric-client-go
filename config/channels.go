...
type ChannelPeer struct {
	...
	// 可选参数
	//
	// 是否会向该节点发送交易以供其背书，节点必须安装configcode。应用程序也可以使用这个属性来决定发送configcode安装请求的节点。
	//
	// 默认值:true
	...
	// 可选参数
	//
	// 这个节点是否可被发送查询建议，节点必须安装configcode。应用程序也可以使用这个属性来决定发送configcode安装请求的节点。
	//
	// 默认值:true
	ChaincodeQuery bool `yaml:"configcodeQuery"`
	// LedgerQuery
	//
	// 可选参数
	//
...
func (c *Channel) set(org *config.Org, channel *config.Channel) {
	if nil != channel.Policies {
		c.setPolicy(channel.Policies)
	}
	if len(channel.PeerNames) > 0 {
		c.setPeers(org, channel)
	}
}

func (c *Channel) setPolicy(in *config.Policy) {
...
func (c *Channel) setQueryChannelConfig(channelConfig *config.QueryChannelConfig) {
	if channelConfig.MinResponses > 0 {
		c.Policies.QueryChannelConfig.MinResponses = channelConfig.MinResponses
	}
	if channelConfig.MaxTargets > 0 {
		c.Policies.QueryChannelConfig.MaxTargets = channelConfig.MaxTargets
	}
	if nil != channelConfig.RetryOpts {
		if channelConfig.RetryOpts.Attempts > 0 {
			c.Policies.QueryChannelConfig.RetryOpts.Attempts = channelConfig.RetryOpts.Attempts
		}
...
func (c *Channel) setDiscovery(discovery *config.Discovery) {
	if discovery.MaxTargets > 0 {
		c.Policies.Discovery.MaxTargets = discovery.MaxTargets
	}
	if nil != discovery.RetryOpts {
		if discovery.RetryOpts.Attempts > 0 {
			c.Policies.Discovery.RetryOpts.Attempts = discovery.RetryOpts.Attempts
		}
...
func (c *Channel) setEventService(eventService *config.EventService) {
	...
	if eventService.BlockHeightLagThreshold > 0 {
		c.Policies.EventService.BlockHeightLagThreshold = eventService.BlockHeightLagThreshold
	}
...
func (c *Channel) setPeers(org *config.Org, channel *config.Channel) {
	for _, peerName := range channel.PeerNames {
